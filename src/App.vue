    ...
    <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
      <!-- <div class="mb-5">
        <nav aria-label="Tabs" class="ec lx aci acn aea bbo text-primary-100">
          <button v-for="(tab, index) in tabs.filter(tab => tab?.isVisible !== false)" :key="index"
      ...
      <h1 class="text-2xl font-bold tracking-tight pt-8 pb-8 ">
        ...
        <h1 class="text-2xl font-bold tracking-tight pt-8 pb-8 text-secondary-600 dark:text-primary-300 ">
        {{ $route?.name }}
      </h1>
